...
public class ValidationHandler {
	...
	public void checkReferenedKey() throws Exception{
		...
		try {
			List<Metadata> list = null;//entity.getMetaData();
			...
			for (String rConstraintName : rConstraintNames) {
				...
				if("R".equals(metadata.getConstraintType())){
					...
					/*try{
						//entities= newdao.read(metadata.getTableName().replace("_", "."), metadata.getRColumn(), BaseDAO.EXPRESSION_EQUALS, foreignKey, true);
						fkentity = newdao.read(metadata.getTableName().replace("_", "."), foreignKey);
					} catch (Exception e) {
						if (newdao != null){
							newdao.close();
						}
	...
	private Metadata getMetadata(String rConstraintName){
		List<Metadata> metaList = null;//entity.getMetaData();
