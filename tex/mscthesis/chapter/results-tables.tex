%ब 

\newcolumntype{B}{>{\columncolor{light-gray}}c} 

	\begin{table}[h]
	\centering
	\caption{Response time in milliseconds per entity}\label{tres:ResponseTime}
		\begin{tabular}{ccBcccc}
		
			\toprule &&\textbf{Baseline} & \textbf{Solution1} & \textbf{Solution2} &
			\textbf{Solution3} & \textbf{Solution4}\\
						
			\midrule \multirow{3}{*}{\textbf{insert}} & \textbf{s} & 0.366 (0.082) & 0.568
			(0.031) & 0.820 (0.091) & 2.108 (0.052) & \TB{0.364 (0.017)}\\
			& \textbf{c} & 0.352 (0.051) & 0.547 (0.044) & 0.803 (0.046) & 2.092 (0.060) &
			\TB{0.351 (0.012)}\\
			& \textbf{e} & 0.305 (0.013) & 1.239 (0.036) & 1.405 (0.022) & 3.484 (0.048) &
			\TB{0.936 (0.013)}\\
						
			\midrule \multirow{3}{*}{\textbf{update}} & \textbf{s} & 0.730 (0.161) & 19.144
			(0.389) & 20.840 (0.426) & 46.394 (0.729) & \TB{14.997 (0.366)}\\
			& \textbf{c} & 0.759 (0.056) & 5.810 (0.498) & 5.991 (0.268) & 10.419 (0.296) &
			\TB{4.751 (0.284)}\\
			& \textbf{e} & 0.404 (0.027) & 1.353 (0.029) & 1.500 (0.024) & 3.579 (0.050) &
			\TB{1.031 (0.013)}\\
						
			\midrule \multirow{3}{*}{\textbf{delete}} & \textbf{s} & 0.314 (0.027) & 7.425
			(0.439) & 10.533 (0.414) & 26.023 (0.553) & \TB{5.638 (0.362)}\\
			& \textbf{c} & 0.287 (0.048) & 2.037 (0.078) & 2.367 (0.085) & 3.958 (0.118) &
			\TB{1.964 (0.086)}\\
			& \textbf{e} & 0.290 (0.027) & 0.410 (0.017) & 0.744 (0.023) & 2.132 (0.042) &
			\TB{0.299 (0.016)}\\
						
			\bottomrule
		\end{tabular}
	\end{table}

	\begin{table}[h]
	\centering
	\caption{Throughput in entities per second}\label{tres:Throughput}
		\begin{tabular}{ccBcccc}
			
			\toprule
			&&\textbf{Baseline} & \textbf{Solution1} & \textbf{Solution2} &
			\textbf{Solution3} & \textbf{Solution4}\\
			
			\midrule \multirow{3}{*}{\textbf{insert}} & \textbf{s} & 2790 (291) & 1764 (85)
			& 1228 (82) & 475 (12) & \TB{2755 (125)}\\
			& \textbf{c} & 2880 (264) & 1837 (112) & 1250 (69) & 478 (13) & \TB{2856
			(96)}\\
			& \textbf{e} & 3282 (116) & 807 (22) & 712 (11) & 287 (4) & \TB{1069 (15)}\\
			
			\midrule \multirow{3}{*}{\textbf{update}} & \textbf{s} & 1394 (121) & 52 (1) &
			48 (1) & 22 (0) & \TB{67 (2)}\\
			& \textbf{c} & 1325 (97) & 173 (14) & 167 (7) & 96 (3) & \TB{211 (12)}\\
			& \textbf{e} & 2483 (119) & 739 (15) & 667 (11) & 279 (4) & \TB{970 (12)}\\
			
			\midrule \multirow{3}{*}{\textbf{delete}} & \textbf{s} & 3198 (205) & 135 (8) &
			95 (4) & 38 (1) & \TB{178 (11)}\\
			& \textbf{c} & 3574 (567) & 492 (19) & 423 (15) & 253 (7) & \TB{510 (21)}\\
			& \textbf{e} & 3470 (206) & 2443 (95) & 1346 (44) & 469 (9) & \TB{3351
			(167)}\\
			\bottomrule
		\end{tabular}
	\end{table}

	\begin{table}[h]
	\centering
	\caption{Response time ratio}\label{tres:ResponsetimeRatio}
		\begin{tabular}{ccBcccc}
			
			\toprule &&\textbf{Baseline$^*$} & \textbf{Solution1} & \textbf{Solution2} &
			\textbf{Solution3} & \textbf{Solution4}\\
						
			\midrule \multirow{3}{*}{\textbf{insert}} & \textbf{s} & 0.37 & 1.55 & 2.24 &
			5.76 & \TB{0.99}\\
			& \textbf{c} & 0.35 & 1.56 & 2.28 & 5.95 & \TB{1.00}\\
			& \textbf{e} & 0.31 & 4.06 & 4.61 & 11.42 & \TB{3.07}\\
						
			\midrule \multirow{3}{*}{\textbf{update}} & \textbf{s} & 0.73 & 26.21 & 28.53 &
			63.51 & \TB{20.53}\\
			& \textbf{c} & 0.76 & 7.66 & 7.90 & 13.74 & \TB{6.26}\\
			& \textbf{e} & 0.40 & 3.35 & 3.71 & 8.86 & \TB{2.55}\\
						
			\midrule \multirow{3}{*}{\textbf{delete}} & \textbf{s} & 0.31 & 23.61 & 33.50 &
			82.76 & \TB{17.93}\\
			& \textbf{c} & 0.29 & 7.09 & 8.24 & 13.78 & \TB{6.84}\\
			& \textbf{e} & 0.29 & 1.41 & 2.57 & 7.36 & \TB{1.03}\\
			
			\bottomrule
			\multicolumn{3}{l}{\scriptsize $^*$ Milliseconds per entity} & &
			\multicolumn{3}{r}{\scriptsize Lower values mean faster response}
		
		\end{tabular}
	\end{table}

\newpage

	\begin{table}[h]
	\centering
	\caption{Throughput Ratio} \label{tres:ThroughputRatio}
		\begin{tabular}{ccBcccc}
			
			\toprule
			&&\textbf{Baseline$^*$} & \textbf{Solution1} & \textbf{Solution2} &
			\textbf{Solution3} & \textbf{Solution4}\\
			
			\midrule
			\multirow{3}{*}{\textbf{insert}} & \textbf{s} & 2790 & 0.63 & 0.44 & 0.17 &
			\TB{0.99}\\
			 & \textbf{c} & 2880 & 0.64 & 0.43 & 0.17 & \TB{0.99}\\
			 & \textbf{e} & 3282 & 0.25 & 0.22 & 0.09 & \TB{0.33}\\
			
			\midrule
			\multirow{3}{*}{\textbf{update}} & \textbf{s} & 1394 & 0.04 & 0.03 & 0.02 &
			\TB{0.05}\\
			 & \textbf{c} & 1325 & 0.13 & 0.13 & 0.07 & \TB{0.16}\\
			 & \textbf{e} & 2483 & 0.30 & 0.27 & 0.11 & \TB{0.39}\\
			
			\midrule
			\multirow{3}{*}{\textbf{delete}} & \textbf{s} & 3198 & 0.04 & 0.03 & 0.01 &
			\TB{0.06}\\
			 & \textbf{c} & 3574 & 0.14 & 0.12 & 0.07 & \TB{0.14}\\
			 & \textbf{e} & 3470 & 0.70 & 0.39 & 0.14 & \TB{0.97}\\
			
			\bottomrule
			\multicolumn{3}{l}{\scriptsize $^*$ Entities per second} & &
			\multicolumn{3}{r}{\scriptsize Higher values mean more throughput}
		\end{tabular}
	\end{table}





