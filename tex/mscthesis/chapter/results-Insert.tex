...
\texttt{Course}), Solution~4 performs almost similar to the baseline while
Solution~3 is still about 5 times slower than the baseline.
In such cases, Solutions~1 and 2 are nearly twice slower than the
baseline. Note that Baseline and Solution~4 are quite similar when
\texttt{Student} and
\texttt{Course} entities are inserted, despite having metadata access in
Solution~4 (Figures~\ref{fres:insert-user} and~\ref{fres:insert-course}). 
% A possible reason for this is that baseline
