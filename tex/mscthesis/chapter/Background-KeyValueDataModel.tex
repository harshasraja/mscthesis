...
(\todo{cite }).  The data model explored in this section is the column-oriented
...
databases as metadata , as seen in Cassandra (\todo{cite DataStax }).  Such
...
and imposing no rigid structures or schema requirements (\todo{cite DataStax and
Cassandra paper}).
...
(\todo{cite }).  Timstamps are used to store the time of the latest update made
