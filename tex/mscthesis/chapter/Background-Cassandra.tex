...
Cisco,  Digg,  Reddit, and others (\todo{Gunda,  2010}). 
...
configured to operate together and run as a single cluster  (\todo{Perham, 2010a,  BOOK}).
...
multiple data centers,  even if these  are geographically distributed(\todo{
BOOK}).  The details of such a distributed nature is abstracted from  the user
such that 
...
their status (\todo{cite Cassandra paper}). Such communication between the nodes
...
performed(\todo{cite Cassandra papers}).
...
and Amazon's Dynamo (\todo{cite}).  Over time, these adopted concepts evolved
...
position in the ring is larger than the hashed value of the key (\todo{cite
book}).
...
balanced and distributed in the cluster  irrespective of addition or removal of nodes
(\todo{cite Book,  most Cassandra papers}). 
...
is set to the number of nodes on which the application wants to create replicas
(\todo{cite Book, most Cassandra papers}).  The replication factor tells the
cluster how many copies to create of a single data item.
...
propagated to all the replicas in a cluster in an asynchronous way (\todo{Henry,
2008}).
...
data in the cluster(\todo{Perham, 2010a and cassandra paper}). .
